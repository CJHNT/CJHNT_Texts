...
for file in files:
    ...
    if not es.indices.exists(new_indices[coll]['new']):
        ...
        es.indices.create(index=new_indices[coll]['new'], body={"settings": {"index": {"number_of_shards": 1},
                                                                "analysis": {"filter": auto_filter["filter"],
                                                                             "analyzer": auto_analyzer["analyzer"]}},
...
for k, v in new_indices.items():
    ...
    es.indices.put_alias(index=v['new'], name=coll_type.get(k, 'jewish'))
